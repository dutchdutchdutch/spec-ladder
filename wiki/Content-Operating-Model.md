...
**How each touchpoint renders the string.** Web, mobile, email, SMS. Where a string wraps, where it truncates, and how long it may run belong to `DESIGN.md` and to the build check that enforces them, not to the artifact that says what the words are.
